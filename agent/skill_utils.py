...
def get_all_skills_dirs() -> list[Path]:
    ...
    When ``ZHUR_AI_AGENT_ROOT`` (or the install tree) exposes ``skills/``, that
    directory is appended last.
    """
    dirs = [get_skills_dir()]
    dirs.extend(get_external_skills_dirs())
    try:
        from agent.ecosystem_paths import get_ecosystem_skills_dir

        eco_skills = get_ecosystem_skills_dir()
        if eco_skills is not None:
            resolved = eco_skills.resolve()
            if resolved not in {d.resolve() for d in dirs}:
                dirs.append(resolved)
    except Exception:
        logger.debug("Ecosystem skills dir lookup failed", exc_info=True)
    return dirs
