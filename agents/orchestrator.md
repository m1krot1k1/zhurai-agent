...
Если у nested orchestrator нет `Task()` → `ORCHESTRATOR_RELAY_REQUEST` родителю (§0.1.3, `docs/delegation-chain.md`). ЗАПРЕЩЕНО делать работу самому (Read/Shell/git) — это `MULTI_AGENT_PIPELINE_BLOCKED` semantics.
...
- **Nested orchestrator, `Task()` недоступен** → `ORCHESTRATOR_RELAY_REQUEST` родителю (`rules/orchestrator.mdc` §0.1.3; `docs/delegation-chain.md`). Relay path — не single-agent execution.
- **Fallback на Read/Shell/git вместо relay** → HARD FAIL → `MULTI_AGENT_PIPELINE_BLOCKED: Task tool unavailable` (или `status: blocked`, `execution_state: blocked`, `reason: MULTI_AGENT_PIPELINE_BLOCKED`).
- **`DELEGATION_BLOCKED`** — deprecated input alias only; на wire нормализуется в `MULTI_AGENT_PIPELINE_BLOCKED`.
- **Режим /start, swarm, 24/7, open-ended + нет Task + нет relay path** → не деградировать в single-agent; только `MULTI_AGENT_PIPELINE_BLOCKED`.
