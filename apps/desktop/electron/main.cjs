...
if (IS_WINDOWS) {
  app.setAppUserModelId('com.m1krot1k1.zhurai-agent')
}
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 m1krot1k1'
})
...
// Repo-sourced agent registry: scans ZHUR_AI_AGENT_ROOT/agents/*.md (or hermesRoot).
ipcMain.handle('hermes:agent-ecosystem:list', async () => discoverRepoAgents({ hermesRoot: resolveUpdateRoot() }))
...
// Packaged .app: show Electron/desktop release version (matches GitHub tag).
// Dev/source: prefer hermes_cli __version__ when the backend checkout is readable.
