...
### ZCode Delegation Patterns

Agent-manager orchestrates lifecycle operations by delegating to domain specialists via ZCode sub-session delegation. Below are the primary delegation scenarios.
...
```
ZCode sub-session with `../agents/subagent-factory.md` brief:
 ...
 SCOPE:
  ...
  - ../rules/release-manager.md (domain rules)
  - references/skills/release-manager.md (reference skill)
 OUT_OF_SCOPE: Orchestrator routing rules, other agents, CI/CD scripts
 OWNERSHIP: ../agents/release-manager.md, ../rules/release-manager.md, references/skills/release-manager.md
 DEPENDENCIES: none
 STEPS:
  ...
  3. Create ../rules/release-manager.md with domain-specific protocols (semver, changelog generation, npm/pypi publish flow)
  4. Create references/skills/release-manager.md with step-by-step release workflow
  5. Verify file consistency across all three artifacts
 DELIVERABLES:
  - ../agents/release-manager.md — complete agent spec with CHEAT SHEET
  - ../rules/release-manager.md — release-domain protocols
  - references/skills/release-manager.md — release workflow reference
 ACCEPTANCE_CRITERIA:
...
```
ZCode sub-session with `../agents/agent-architect.md` brief:

 OBJECTIVE: Update database-specialist to add migration rollback protocol and dry-run planning.
 SCOPE:
  - ../rules/database-specialist.md (add rollback + dry-run sections)
  - database-specialist.md (add rollback workflow)
 OUT_OF_SCOPE: ../agents/database-specialist.md (structural changes only if CHEAT SHEET requires expansion), other agents
 OWNERSHIP: ../rules/database-specialist.md, database-specialist.md
 DEPENDENCIES: none
 STEPS:
  1. Read the current ../rules/database-specialist.md and database-specialist.md
 ...
 DELIVERABLES:
  - ../rules/database-specialist.md — updated with rollback and dry-run sections
  - database-specialist.md — updated with rollback workflow
 ACCEPTANCE_CRITERIA:
...
```
ZCode sub-session with `../agents/repo-explorer.md` brief:
 ...
 SCOPE:
  ...
  - ../rules/ directory (rule *.md files, excluding shared files like orchestrator.md and coding-guardrails.md)
  - references/skills/ directory (reference skill *.md files)
 OUT_OF_SCOPE: ../orchestration/, scripts/, runtime mirror
 OWNERSHIP: none (read-only)
 STEPS:
  1. Extract the routing table from ../rules/specialists.mdc: agent name, referenced file paths
  2. List all files in ../agents/, ../rules/, references/skills/ directories
  3. Cross-reference: for each agent in the routing table, verify:
     a. ../agents/{name}.md exists
     b. ../rules/{name}.md exists (if agent has domain-specific rules)
     c. references/skills/{name}.md exists (when applicable)
 ...
 ACCEPTANCE_CRITERIA:
  ...
  - [ ] Report structure: {agent_name: {agent_md: present|missing, rules_md: present|missing|n/a, skill_md: present|missing}}
 NON-NEGOTIABLE:
