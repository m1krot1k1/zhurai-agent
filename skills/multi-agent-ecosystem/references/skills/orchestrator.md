...
Обеспечить повторяемый, проверяемый конверт для любого ZCode delegation envelope / ZCode delegation envelope: однозначный scope, изоляция владения файлами, явные зависимости между ветками и завершаемый completion contract. Согласовать терминологию с каноническими правилами и документами цепочки делегирования.
...
#### Pre-flight (вход — проверяет orchestrator перед ZCode sub-session delegation):
...
1. Прочитать актуальные требования в `../rules/orchestrator.mdc` (completeness gate §3) и `../orchestration/delegation-chain.md` для вашего режима (root vs nested).
...
- [ ] Терминология согласована с `../orchestration/delegation-chain.md` и `../orchestration/start-workflow.md`

### СВЯЗАННЫЕ ДОКУМЕНТЫ (шаблоны)

- **`../rules/orchestrator.mdc`** — completeness gate, parallel writer safety, proof-of-delegation
- **`../orchestration/delegation-chain.md`** — детальная цепочка и relay
- **`../orchestration/evidence-first-acceptance.md`** — матрица claim/evidence для приёмки
- См. реестр: `INDEX.md` (forward ref)
- Смежно: `structured-policy-yaml.md` для машиночитаемых фрагментов политики в задачах
...
- Authoring or reviewing ZCode delegation envelope / orchestration packets
