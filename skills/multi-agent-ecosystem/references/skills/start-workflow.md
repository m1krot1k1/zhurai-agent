...
> ZCode sub-session delegation при этом доступен вложенным subagent'ам; ограничения задаются ролью и правилами, а не глубиной как таковой.

```
/start (root supervisor)
   └→ orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)  ← ПЕРВЫЙ И ЕДИНСТВЕННЫЙ tool call
        ├→ ZCode sub-session (`../agents/code.md`)
        ├→ ZCode sub-session (`../agents/test-specialist.md`)
        ├→ ZCode sub-session (`../agents/security-auditor.md`)
        └→ orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)  ← sub-orchestrators для сложных задач
```

### FIRST_ACTION Gate (КРИТИЧЕСКИ ВАЖНО)

- Root-start: **НОЛЬ tool calls** до спавна orchestrator
- Первый tool call = `orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)`
- До этого ЗАПРЕЩЕНО: Read, Write, Grep, Shell, SemanticSearch, start router via `/start`
...
|---|---|
| `start router via `/start`` | Legacy hop: нарушает flat-chain и размывает ответственность root-start |
| Root-start → Read/Write → ZCode sub-session with `../agents/start.md` brief ("acknowledge") | Фейковая делегация |
...
При `until_user_stop` root-start запускает `orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)` для каждой волны:
```
Волна 1: orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`) → результат
Волна 2: orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`) → результат  ← ОБЯЗАТЕЛЬНО!
```
...
| Проблема | Решение |
|---|---|
| start читает файлы сам | **НАРУШЕНИЕ** — только orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`) |
| В цепочке появился worker-start | **Legacy drift** — вернуться к `root-start → orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)` |
| Orchestrator не вызван | **НАРУШЕНИЕ** — FIRST_ACTION = orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`) |
| 24/7 не пошла волна 2 | Проверить loop в root-start — ОБЯЗАН запускать следующую волну |
| Specialist делает всё сам | **НАРУШЕНИЕ** — Mandatory SWARM: ZCode sub-sessions для каждой подзадачи |

## ЧЕКЛИСТ

- [ ] FIRST_ACTION: первый tool call = orchestrator mode via `/orchestrator` (load `../orchestration/delegation-chain.md`)
...
- [ ] Specialists используют ZCode sub-sessions для подзадач (Mandatory SWARM)
...
2. Log the approval reference in completion contract (quote user line or task ID)—pair with `structured-agent-logging.md` when traces are required.
